...
import (
	...
	"github.com/XinFinOrg/XDPoSChain/core/rawdb/ancienttest"
...
// TestFreezerNewTableAlignment exercises the new-table alignment path: a freezer
// is first opened with a subset of the table set, populated with a few items and
// closed. It is then re-opened with an additional table. The expectation is that
// the new table is fast-forwarded to the existing head without disturbing the
// pre-existing tables, and that subsequent writes append cleanly across all of
// them.
//
// Upstream go-ethereum covers this with the EIP-7928 BAL table, which this fork
// does not port (see claude/plan 2.2). A synthetic "extra" table is used instead
// so the alignment mechanism itself stays covered.
func TestFreezerNewTableAlignment(t *testing.T) {
	const (
		extraTable = "extra"
	...
	// The "legacy" table set: everything the chain freezer has today.
	legacyTables := make(map[string]freezerTableConfig)
	for name, cfg := range chainFreezerTableConfigs {
		legacyTables[name] = cfg
	}
	// The "upgraded" table set adds one more table in its own tail group.
	fullTables := make(map[string]freezerTableConfig)
	for name, cfg := range legacyTables {
		fullTables[name] = cfg
	}
	fullTables[extraTable] = freezerTableConfig{noSnappy: false, tailGroup: extraGroup}
	...
	if _, err := f.ModifyAncients(func(op ethdb.AncientWriteOp) error {
		for i := uint64(0); i < items; i++ {
			for _, kind := range []string{
				...
				if err := op.AppendRaw(kind, i, payload); err != nil {
					return err
				}
			}
		}
	...
	if err := f.Close(); err != nil {
		t.Fatalf("close legacy freezer: %v", err)
	}

	// Re-open with the extra table present.
	f, err = NewFreezer(dir, "", false, 2049, fullTables)
	if err != nil {
		t.Fatalf("can't reopen freezer with new table: %v", err)
	}
	defer f.Close()

	if got, _ := f.Ancients(); got != items {
		t.Fatalf("head after realignment: got %d, want %d", got, items)
	}
	// Pre-existing tables must be intact.
	for i := uint64(0); i < items; i++ {
		for _, kind := range []string{
			ChainFreezerHashTable, ChainFreezerHeaderTable,
			ChainFreezerBodiesTable, ChainFreezerReceiptTable,
	...
	// The new table's tail should equal the head - it is empty but aligned.
	if tail, err := f.Tail(extraGroup); err != nil || tail != items {
		t.Fatalf("extra tail: got %d (err %v), want %d", tail, err, items)
	}
	// Reads of the new table for any pre-alignment item must report out-of-bounds.
	for i := uint64(0); i < items; i++ {
		if _, err := f.Ancient(extraTable, i); err == nil {
			t.Fatalf("reading extra[%d] succeeded; want error (out of bounds)", i)
		}
	}
	// A subsequent batch must append uniformly to every table.
	extraPayload := []byte("real-extra")
	if _, err := f.ModifyAncients(func(op ethdb.AncientWriteOp) error {
		i := items
		for _, kind := range []string{
			ChainFreezerHashTable, ChainFreezerHeaderTable,
		...
		return op.AppendRaw(extraTable, i, extraPayload)
	}); err != nil {
	...
	got, err := f.Ancient(extraTable, items)
	if err != nil {
		t.Fatalf("extra[%d]: %v", items, err)
	}
	if !bytes.Equal(got, extraPayload) {
		t.Fatalf("extra[%d]: got %x, want %x", items, got, extraPayload)
	}
}
