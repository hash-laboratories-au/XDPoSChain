...
// freezeThreshold returns the threshold for chain freezing, namely
// HEAD-params.FullImmutabilityThreshold.
//
// Deviation from upstream go-ethereum: upstream uses max(finality, headLimit),
// where finality comes from ReadFinalizedBlockHash (a post-merge beacon concept).
// XDPoS has its own finality and stores no such marker, so only the immutability
// depth is used here. This is strictly more conservative than upstream.
func (f *chainFreezer) freezeThreshold(db ethdb.KeyValueReader) (uint64, error) {
	...
	return head - params.FullImmutabilityThreshold, nil
}
...
func (f *chainFreezer) freeze(db ethdb.KeyValueStore) {
	...
	for {
		...
		for i := 0; i < len(ancients); i++ {
			...
			if first+uint64(i) != 0 {
				deleteFrozenCanonicalBlock(batch, ancients[i], first+uint64(i))
...
func (f *chainFreezer) Ancient(kind string, number uint64) ([]byte, error) {
	...
	// The item has been pruned (minimal history mode). Upstream go-ethereum
	// would fall back to an Era store here; this fork does not port that layer,
	// so a pruned item is simply reported as out of bounds and callers fall
	// back to the key-value store (which returns nil for pruned data).
...
// deleteFrozenCanonicalBlock removes the key-value copies of a canonical block
// whose data has just been written to the ancient store.
//
// It deliberately does NOT delete the total difficulty. Upstream go-ethereum can
// drop TD here because it copies TD into a "diffs" ancient table; this fork does
// not freeze TD at all (see claude/plan 3.4), so deleting it here would destroy
// it permanently. TD is small and stays in the key-value store for the life of
// the chain.
//
// The hash->number mapping is likewise preserved: ancient blocks are still looked
// up by hash.
func deleteFrozenCanonicalBlock(db ethdb.KeyValueWriter, hash common.Hash, number uint64) {
