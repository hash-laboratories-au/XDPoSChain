...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case msg.Code == NewBlockHashesMsg:
		...
		for _, block := range unknown {
			log.Info("handler receive NewBlockHashesMsg", "Hash", block.Hash.Hex(), "Number", block.Number)
	...
	case msg.Code == NewBlockMsg:
		...
		log.Info("handler receive NewBlockMsg", "Hash", request.Block.Hash().Hex(), "Number", request.Block.Number())
...
func (pm *ProtocolManager) BroadcastVote(vote *types.Vote) {
	...
	if len(peers) > 0 {
		for _, peer := range peers {
			...
			if err != nil {
				log.Error("[BroadcastVote] Fail to broadcast vote message", "err", err, "peerId", peer.id, "version", peer.version, "blockNum", vote.ProposedBlockInfo.Number)
...
func (pm *ProtocolManager) BroadcastTimeout(timeout *types.Timeout) {
	...
	if len(peers) > 0 {
		for _, peer := range peers {
			...
			if err != nil {
				log.Error("[BroadcastTimeout] Fail to broadcast timeout message, remove peer", "err", err, "peerId", peer.id, "version", peer.version, "timeout", timeout)
				pm.removePeer(peer.id)
			}
		}
...
func (pm *ProtocolManager) BroadcastSyncInfo(syncInfo *types.SyncInfo) {
	...
	if len(peers) > 0 {
		for _, peer := range peers {
			...
			if err != nil {
				log.Error("[BroadcastSyncInfo] Fail to broadcast syncInfo message, remove peer", "err", err, "peerId", peer.id, "version", peer.version, "syncInfo", syncInfo)
				pm.removePeer(peer.id)
			}
		}
