...
var (
	...
	// errUnknownTable is returned if a table is being accessed which is not tracked
	// by the freezer.
	errUnknownTable = errors.New("unknown table")

	// errOutOrderInsertion is returned if a fresh item is inserted out of order
	// into the freezer table.
	errOutOrderInsertion = errors.New("the append operation is out-order")

	// errSymlinkDatadir is returned if the ancients directory is a symbolic link.
...
// freezerTable represents a single chained data table within the freezer (e.g.
// headers or bodies). It consists of a data file (snappy-compressed or raw)
// segmented by file number, plus an index file that maps logical item numbers
// to (filenum, end-offset) tuples.
type freezerTable struct {
	// WARNING: The `items` field is accessed atomically. On 32 bit platforms,
	// only 64-bit aligned fields can be atomic. The struct is guaranteed to be
	// so aligned, so take advantage of that. For more information, see:
	...
	head   *os.File            // File descriptor for the data head file (name.X.rdat / cdat)
	...
	sizeGauge  *metrics.Counter // Gauge for tracking the combined size of all freezer tables

	logger log.Logger   // Logger describing the db
...
func newTable(path string, name string, readMeter *metrics.Meter, writeMeter *metrics.Meter, sizeGauge *metrics.Counter, disableSnappy bool) (*freezerTable, error) {
	// Ensure the containing directory exists and isn't a symlink
	...
	// Compose the filenames for opening the index file and head data file.
	var idxName string
	if disableSnappy {
	...
	tab := &freezerTable{
		...
		maxFileSize:   2 * 1000 * 1000 * 1000, // 2 GB segment cap
	}
...
// repair cross-checks the head and the index file and truncates them to be in
// sync with each other after a potential crash / data loss.
func (t *freezerTable) repair() error {
	...
	for contentExp != contentSize {
		...
		if contentExp > contentSize {
			t.logger.Warn("Truncating dangling indexes", "indexed", contentExp, "stored", contentSize)
...
// preopen opens all files that are readable, but only keeps the head file open
// for writing.
...
func (t *freezerTable) truncate(items uint64) error {
	...
	if expected.filenum != t.headId {
		...
		// Release any files _after_ the new head -- both the previous head
		// (which is now beyond the new head) and any further already-open files.
		t.releaseFilesAfter(expected.filenum, true)
		// The new head file is also already open, fix that
		t.head = newHead
	}
...
// fsync before crucial operations.
func (t *freezerTable) Append(item uint64, blob []byte) error {
	...
	if atomic.LoadUint64(&t.items) != item {
		t.lock.RUnlock()
		return fmt.Errorf("%w: have %d, want %d", errOutOrderInsertion, atomic.LoadUint64(&t.items), item)
	}
	...
	if t.headBytes+bLen < bLen ||
		...
		// We need to start a new file. Release the RLock and acquire WLock.
		t.lock.RUnlock()
		t.lock.Lock()
		...
		// Create a new head file
		nextID := t.headId + 1
		newHead, err := t.openFile(nextID, openFreezerFileForAppend)
		if err != nil {
			t.lock.Unlock()
			return err
		}
...
// getBounds returns the indexes for the item at position number.
// It returns the indexEntry that points to that data item, and the indexEntry
// that points to its predecessor (which is used to detect the beginning of the item).
...
// Note: not O_APPEND because it interacts badly with Truncate() on some OSes.
...
// openFreezerFileTruncated opens (creates) a freezer table file and truncates it to zero length.
