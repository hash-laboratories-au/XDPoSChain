...
func (x *XDPoS_v2) YourTurn(chain consensus.ChainReader, parent *types.Header, signer common.Address) (bool, error) {
	...
	if waitedTime < int64(x.config.V2.MinePeriod) {
		log.Warn("[YourTurn] wait after mine period", "minePeriod", x.config.V2.MinePeriod, "waitedTime", waitedTime)
		return false, nil
	}
...
func (x *XDPoS_v2) IsEpochSwitch(header *types.Header) (bool, uint64, error) {
	...
	log.Info("[IsEpochSwitch]", "is", parentRound < epochStartRound, "parent round", parentRound, "round", round, "number", header.Number.Uint64(), "epochNum", epochNum, "hash", header.Hash())
